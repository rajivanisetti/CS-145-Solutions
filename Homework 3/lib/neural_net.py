...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        def MSE_loss(y_pred, y_target):
            ...
            y_target_matrix = np.zeros((num_samples, num_attributes))
            for i in range(num_samples):
                y_target_matrix[i][y_target[i]] = 1
            diff = y_pred - y_target_matrix
            dy_pred = diff / num_samples
        ...
        # ================================================================ #
        sum_one = np.sum(np.square(self.params['W1']))
        sum_two = np.sum(np.square(self.params['W2']))
        reg_loss = 0.5 * reg * (sum_one + sum_two) 
