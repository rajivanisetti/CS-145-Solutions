...
class SVM(object):
    ...
    def project(self, X):
        if self.w is not None: ## linear case
            ...
            kernelMat = np.zeros((len(X), len(self.sv)))
            for indexOne in range(len(X)):
                for indexTwo in range(len(self.sv)):
                    kernelMat[indexOne][indexTwo] = self.kernel_point(self.sv[indexTwo], X.transpose()[indexOne]).transpose()
        
            operatorOne = kernelMat * self.sv_y
            y_predict = np.dot(operatorOne, self.a)
