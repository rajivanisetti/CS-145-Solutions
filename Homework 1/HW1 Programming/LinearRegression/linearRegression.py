...
def getBeta(train_x, train_y):   
    ...
    beta = np.zeros(p)
    termOne = np.linalg.inv(np.matmul(np.transpose(train_x), train_x))
    termTwo = np.matmul(np.transpose(train_x), train_y)
    beta = np.matmul(termOne, termTwo)
...
def getBetaStochasticGradient(train_x, train_y, lr):
    ...
    for iter in range(epoch):
        ...
        for i in range(n):
            idx = indices[i]
            beta = beta + ((lr * (train_y[idx] - np.matmul(np.transpose(train_x[idx,:]), beta))) * train_x[idx,:])
    return beta
