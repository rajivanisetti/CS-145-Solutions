...
def compute_avglogL(X,y,beta):
    ...
    for i in range(n):
        xT = X[i, :]
        firstTerm = -1 * y[i] * xT.dot(beta)
        logL += firstTerm + np.log(eps + 1 + np.exp(xT.dot(beta)))
...
def getBeta_BatchGradientRegularized(train_x, train_y, lr, num_iter, verbose, lambdaMag):
    beta = np.random.rand(train_x.shape[1])
    ...
    for iter in range(0, num_iter):
        ...
        deriv = np.random.rand(p)
        for i in range(p):
            deriv[i] = train_x[:, i].dot(diff) + (2 * lambdaMag * beta[i])
        beta += (deriv / n).dot(lr)
...
def getBeta_Newton(train_x, train_y, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        ...
        hessian = np.matmul(np.matmul(-1 * np.transpose(train_x), np.diag(sig * (1 - sig))), train_x)
        beta -= (np.linalg.pinv(hessian).dot(deriv)) / n
...
class LogisticRegression(object):
    ...
    def train(self, algType):
        ...
        if(algType == '0'):
            ...
            beta = getBeta_BatchGradientRegularized(newTrain_x, self.train_y.values, self.lr, self.num_iter, self.verbose, self.lambdaMag)
            print('Beta: ', beta)
        else:
            print('Incorrect beta_type! Usage: 0 - batch gradient descent, 1 - Newton-Raphson method, 2 - batch gradient descent with regularization')
