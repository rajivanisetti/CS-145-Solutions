...
def EStep():
    for i in range(0, N):
        for j in range(0, M):
            ...
            for k in range(0, K):
                p[i, j, k] = float(beta[k, j] * theta[i,k]) / denom
...
def MStep():
    for k in range(0, K):
        denom = float(0)                # Get denominator
        for w in range(0, M):
            for d in range(0, N):
                denom += p[d, w, k] * X[d, w]
        
        for w in range(0, M):           # For each word
            num = float(0)
            for d in range(0, N):       # Sum numerator
                num += p[d, w, k] * X[d, w]
            beta[k, w] = num / denom 
...
def LogLikelihood():
    ...
    for i in range(0, N):
        for j in range(0, M):
            ...
            factor = float(0)
            for k in range(0, K):
                factor += theta[i, k] * beta[k, j]

            loglikelihood += X[i, j] * math.log(factor)
            # ================== YOUR CODE HERE ==========================
