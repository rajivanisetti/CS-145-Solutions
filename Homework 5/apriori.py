...
def gen_one_item_cand_set(input_fileator):
    ...
    for record in input_fileator:
        ...
        for item in transaction:
            ...
            singleSet = set()
            singleSet.add(item)
            if frozenset(singleSet) not in one_cand_set:
                one_cand_set.add(frozenset(singleSet))
...
def run_apriori_loops(one_cand_set, set_count_map, all_transactions,
    ...
    while len(current_l_set) > 0:
        ...
        current_c_set =  join_set(current_l_set, i+1)
...
def get_support(set_count_map, item):
    ...
    return set_count_map[item]
    """
    count = 0
    for set in set_count_map:
        if item.issubset(set) or item is set:
            count += set_count_map[set]
...
def join_set(s, l):
    ...
    for setOne in s:
        for setTwo in s:
            if setOne is not setTwo:
                jointSet = setOne.union(setTwo)
            
                if len(jointSet) == l:
                    totalSets.add(jointSet)
            """
            if l == 1 and setOne is not setTwo:
                minset = set(sortedOne) if sortedOne[0] < sortedTwo[0] else set(sortedTwo)
                addition = sortedTwo[0] if sortedOne[0] < sortedTwo[0] else sortedOne[0]
                minset.add(addition)
                print ("FrozenSet!: {}".format(frozenset(minset)))
                totalSets.add(frozenset(minset))
            elif sortedOne[0:l-1] == sortedTwo[0:l-1] and setOne is not setTwo:
                print("Mult: {}, Two: {}".format(sortedOne[0:l-1], sortedTwo[0:l-1]))
                minset = set(sortedOne) if sortedOne[l-1] < sortedTwo[l-1] else set(sortedTwo)
                addition = max(sortedOne[l-1], sortedTwo[l-1])
                minset.add(addition)
                print ("FrozenSet!: {}".format(frozenset(minset)))
                totalSets.add(frozenset(minset))
            """
